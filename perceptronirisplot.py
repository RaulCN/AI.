...
# Plotando a linha de decisão encontrada pelo Perceptron
x_decision = np.array([np.min(X[:, 0]), np.max(X[:, 0])])
y_decision = (-ppn.w_[0] - ppn.w_[1] * x_decision) / ppn.w_[2]
plt.plot(x_decision, y_decision, color='green', linestyle='-', linewidth=2, label='Linha de decisão')
